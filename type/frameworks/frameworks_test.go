...
func TestFrameworksDataType(t *testing.T) {
...
func TestFrameworksFields(t *testing.T) {
	// Initialize the DataType
	if err := Initialize(); err != nil {
		t.Skipf("Failed to initialize DataType: %v", err)
	}
	...
	if DataType == nil {
		t.Skip("No frameworks data found")
	}

	// Test that we can access frameworks fields
	if len(DataType.Item) == 0 {
		t.Log("No frameworks found (this is normal if no frameworks are available)")
		return
	}

	// Test that each framework has a name
	for i, framework := range DataType.Item {
		if framework.Name == "" {
			t.Errorf("Framework %d should have a name", i)
		}
	}
}
